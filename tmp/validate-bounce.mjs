...
const outputDir = path.resolve("output/web-game/bounce-browser");
...
for (let index = 0; index < 42; index += 1) {
  ...
  if (
    airborneState &&
    !reboundState &&
    parsed.vehicle?.grounded === false &&
    Number(parsed.vehicle?.verticalSpeedKmh ?? 0) > 0
  ) {
    reboundState = parsed;
    await page.screenshot({
      path: path.join(outputDir, "shot-rebound.png"),
      fullPage: true,
    });
    fs.writeFileSync(
      path.join(outputDir, "state-rebound.json"),
      JSON.stringify(parsed, null, 2),
    );
    continue;
  }
...
if (!airborneState || !reboundState || !landingState) {
  ...
  throw new Error("Bounce validation did not observe an airborne, rebound, and settled landing sequence.");
}
